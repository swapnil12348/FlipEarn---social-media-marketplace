...
var InngestExecutionEngine = class extends require_InngestExecution.InngestExecution {
	...
	createCheckpointHandlers() {
		...
		const syncHandlers = {
			...
			"steps-found": async ({ steps }) => {
				...
				delete this.state.executingStep;
				await this.checkpoint([stepToResume]);
			},
	...
	*/
	isFinalAttempt(error) {
		if (error instanceof require_NonRetriableError.NonRetriableError || error?.name === "NonRetriableError") return true;
		return Boolean(this.fnArg.maxAttempts && this.fnArg.maxAttempts - 1 === this.fnArg.attempt);
	}
	/**
